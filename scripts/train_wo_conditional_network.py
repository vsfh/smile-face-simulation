...
def get_pipeline():
    train_loader = get_loader(data_path, 4, 'train', type='pair')
    test_loader = get_loader(data_path, 1, 'test', type='pair')
    net = pSp(decoder_checkpoint_path, start_from_avg=start_from_avg)
    net.load_state_dict(torch.load('/data/shenfeihong/smile/toc_smile/3.29/checkpoint/16000.pt'))
...
def train():
    ...
    while step_idx < max_step:
        for i, batch in enumerate(train_loader):
            ...
            if start_from_avg:
                fake_img, latent = net.forward(cond_img, styles='wo_condition', return_latents=False, concat_img=False)
            else:
                fake_img, _ = net.forward(cond_img, styles='wo_condition', return_latents=False, concat_img=False)
