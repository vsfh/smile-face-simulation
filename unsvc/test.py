...
if __name__=='__main__':
    def a():
        case = 'C01002721350'
        eu = cv2.imread(f'/window/data/smile/out1/{case}/upper_edge.png')
        ed = cv2.imread(f'/window/data/smile/out1/{case}/down_edge.png')
        mk = cv2.imread(f'/window/data/smile/out1/{case}/mouth_mask.png')
        tk = cv2.imread(f'/window/data/smile/out1/{case}/teeth_mask.png')
        img = cv2.imread(f'/window/data/smile/out1/{case}/smile.png')
        depth = cv2.imread(f'/window/data/smile/out1/{case}/step/depth.png')
        ...
        b = img.copy()
        b[tk==0] = 0
        cv2.imwrite(f'/window/data/smile/out1/{case}/step/input_style.png', a)
        cv2.imwrite(f'/window/data/smile/out1/{case}/step/edge.png', eu+ed)
    ...
    def b():
        ...
        face = cv2.imread('/window/data/smile/TeethSimulation/C01002722687/微笑像.jpg')
    ...
    def f():
        def concat(case):
            ...
            # 读取底层图片和顶层图片
            base_image = cv2.imread(f'/window/data/smile/out1/{case}/smile.png')
            overlay_image = cv2.imread(f'/window/data/smile/out1/{case}/step/3d.png')
            a = 0.49
            ...
            cv2.imwrite(f"/window/data/smile/out1/{case}/target.png", new_img.astype(np.uint8))
        ...
        for case in os.listdir('/window/data/smile/out1/'):
            case = 'C01002721350'
            concat(case)
            break
