...
def render_depth_mask(case, step_idx=-1, show=False):
    ...
    best_params = torch.load(f'/mnt/d/data/smile/out/{case}/para.pt')
    step = [file for file in natsort.natsorted(os.listdir(f'/mnt/d/data/smile/Teeth_simulation_10K/{case}')) if file.endswith('txt')][step_idx]
...
def render_3d(case, step=-1, show=False):
    best_params = torch.load(f'/mnt/d/data/smile/out/{case}/para.pt')
    step_idx = [file for file in natsort.natsorted(os.listdir(f'/mnt/d/data/smile/Teeth_simulation_10K/{case}')) if file.endswith('txt')][step]
    os.makedirs(f'/mnt/d/data/smile/out/{case}/step', exist_ok=True)
